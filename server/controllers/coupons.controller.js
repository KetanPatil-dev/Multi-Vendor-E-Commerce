...
export const ValidateCoupon=async(req ,res)=>{
    try {
        const {code}=req.body;
        const coupon=await CoupounModel.find({code:code,userId:req.user._id,isActive:true})
        if(!coupoun)
        {
        ...
        res.json({message:"Valid Coupon",code:coupon.code,discountPercentage:coupon.discountPercentage})
    } catch (error) {
